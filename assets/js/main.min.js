...
(function() {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting && !wasInView) {
        ...
        console.log('🎵 Entered gallery section - starting audio');
        if (!isPlaying) {
          playAudio();
        }
      } else if (!entry.isIntersecting && wasInView) {
        // Just left gallery section
        console.log('🔇 Left gallery section - stopping audio');
        if (isPlaying) {
          pauseAudio();
        }
  ...
  function checkGalleryVisibility() {
    ...
    if (isVisible !== wasInView) {
      ...
      if (isVisible && !wasInView) {
        console.log('📜 Scroll detected: Entered gallery - starting audio');
        if (!isPlaying) {
          playAudio();
        }
      } else if (!isVisible && wasInView) {
        console.log('📜 Scroll detected: Left gallery - stopping audio');
        if (isPlaying) {
          pauseAudio();
        }
        if (audioBanner) {
